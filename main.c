...
int main() {
    ...
    pobj32_2t *object = pobj32_2t_create(&triangle, 1, 10, 0.1);
    ...
    draw_gl *triangle_draw = draw_gl_init_pobj32_2t(object);

    uint32_t width, height;
    //pobj32_2t_add_pulse(object, &vert_add);
    object->vertex_spd[0].x = 0.01;
    object->vertex_spd[1].x = 0.01;
    object->vertex_spd[2].x = -0.01;
    object->vertex_spd[3].x = -0.01;
    ...
    while (!window_gl_should_close(PWindow)) {
        ...
        draw_gl_update_pobj32_2t(triangle_draw, object);

        window_gl_draw(PWindow, main_line, triangle_draw, &trans);

        pobj32_2t_update(object, 0.02);
